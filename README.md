# jeffy-skills

这是一个用于沉淀个人研究方法、汇报生成方法与自动化交付流程的 Skills 仓库。每个 Skill 独立放在自己的子目录中。

## 当前 Skills

| Skill | 说明 |
|---|---|
| `jf-analysis` | 使用立体分析法生成结构化 Markdown 与 PDF 深度研究报告。 |
| `excellent-operations-ppt` | 基于 DESIGN.md 生成规范企业汇报材料，同时输出 PPTX 和 HTML 预览。 |
| `travelplanner` / `travel-planner` | 基于真实公开信息和用户评价生成非特种兵旅行规划，默认输出可交互 HTML，支持 3/5/7 天路线、动态预算、地图、真实商户信息和准备事项。 |
| `global-architecture-diagram` | 将报告、概念、问题或截图转化为完整、结构化、有颜色、有内容的一页式全局架构图。 |

## jf-analysis：立体分析法

`jf-analysis` 采用“立体分析法（3D Analysis）”组织研究报告：

- **X 轴：时间线因果链**：追踪研究对象从起源到当下的关键节点，回答“它是如何一步步走到今天的”。
- **Y 轴：多因素并行展开**：在关键时间截面还原竞品、资本、技术、市场、政策、人物、舆论等力量如何同时作用。
- **Z 轴：力量内部机制拆解**：深入拆解核心力量的内部结构、动机、约束条件和相互影响，解释“为什么会这样走”。

默认交付为结构化 Markdown 研究报告和由 `jf-analysis/scripts/md_to_pdf.py` 生成的 PDF 报告。

## excellent-operations-ppt：卓越运营 PPT

`excellent-operations-ppt` 基于 `excellent-operations-ppt/DESIGN.md` 生成企业汇报材料。它不捆绑 PPTX 模板，而是把 DESIGN.md 作为设计系统规范，用于指导页面版式、组件、配色、排版和质量判断。

默认交付为：

1. `.pptx`：正式 PowerPoint 文件，适合汇报、编辑和二次修改；
2. `.html`：浏览器预览文件，用于快速查看标题链、页面内容和结构。

核心规则：

- 观点在标题上，简短语句表达。
- 每页传递观点三点以内。
- 中文字体微软雅黑，英文字体 Times New Roman。
- 每页字号三种以内。
- 决不允许出现动画和页面转场。
- 页面遵循 DESIGN.md 的视觉规范。
- 关键内容标红加粗。
- 慎用感叹号。

## travelplanner / travel-planner：旅行规划

`travel-planner` 默认按“不要特种兵、尽量领略本地特色”的风格规划行程，优先生成 5 日游；如果用户提供天数、预算、节奏或兴趣偏好，则按用户输入调整。

默认交付为可交互 HTML，覆盖：

- `吃`：本地特色美食、真实餐馆、菜单或价格参考。
- `喝`：精酿、鸡尾酒 bar、咖啡厅。
- `玩乐`：citywalk、徒步、室内攀岩和本地特色活动。
- `购物`：伴手礼和购买地点。
- `住`：兼顾便捷、舒适、干净和交通便利的住宿区域建议。
- `行`：市内交通、城际交通和每日路线。
- `其他`：汇率、准备清单、本地 App、日常用语和注意事项。

## global-architecture-diagram：全局架构图

`global-architecture-diagram` 用于把用户提供的报告、概念、问题、产品想法、技术趋势、业务系统、平台战略、AI/Agent 生态或截图，转化为一张能独立说明问题的全局架构图。

默认输出强调：

- 结构完整：明确用户、输入、理解/处理、能力调用、基础设施、治理和价值结果。
- 视觉清晰：使用分层、流程、生态或能力地图等架构视角，配合颜色、图形和侧栏影响说明。
- 事实边界清楚：区分已核验事实、用户材料和分析推断，避免把自创战略命名包装成官方术语。
- 一图讲清楚：读者应能在 30 秒内回答“这是什么、怎么运作、为什么重要”。
